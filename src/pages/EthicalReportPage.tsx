...
import { Download, Clock, AlertTriangle, Loader2, ChevronDown, ChevronUp, Info, Shield, Eye, Smartphone, Laptop, ChevronLeft, ChevronRight } from 'lucide-react';
...
const EthicalReportPage: React.FC = () => {
    ...
    const getEventTimestamp = (event: CheatingEvent, attemptTimestamp: number) => {
    ...
    useEffect(() => {
        const fetchEthicalReportData = async () => {
            ...
            try {
                ...
                setCurrentEvents(eventsResponse.data[currentAttempt]?.cheating_events || []);
            } catch (error) {
    ...
    const calculateRiskLevel = (events: CheatingEvent[]) => {
        ...
        const highConfidencePhoneEvents = events.filter(e => 
            e.object === "cell phone" && (e.confidence || 0) > 70
        ).length;
        const longLookingEvents = events.filter(e => 
            e.event?.includes("Looking") && (e.duration || 0) > 3
        ).length;

        if (totalEvents === 0) return { level: "Low", color: "#10B981", description: "No suspicious activity detected" };
        
        const riskScore = (highConfidencePhoneEvents * 2) + longLookingEvents;
        
        if (riskScore >= 10) return { level: "Critical", color: "#EF4444", description: "Multiple high-confidence violations detected" };
        if (riskScore >= 5) return { level: "High", color: "#F59E0B", description: "Significant suspicious activity detected" };
        if (riskScore >= 2) return { level: "Medium", color: "#F59E0B", description: "Moderate suspicious activity detected" };
        return { level: "Low", color: "#10B981", description: "Minimal suspicious activity detected" };
    };
    ...
    const downloadPDF = async () => {
        try {
            ...
            header.innerHTML = `
                ...
                <p style="color: #6B7280; font-size: 12px; margin-top: 4px;">Generated: ${format(new Date(), "MMM dd, HH:mm")}</p>
            `;
            ...
            userDetailsDiv.innerHTML = `
                <div style="flex-shrink: 0;">
                    <img src="${userDetails?.profile_picture || 'https://images.unsplash.com/photo-1633332755192-727a05c4013d?w=400&h=400&fit=crop'}" 
                ...
                <div style="flex-grow: 1; display: grid; grid-template-columns: 1fr 1fr; gap: 8px;">
                    <div>
                        <p style="margin: 2px 0; font-size: 11px;"><strong style="color: #4B5563;">Name:</strong> <span style="color: #1F2937;">${userDetails?.username || "N/A"}</span></p>
                        <p style="margin: 2px 0; font-size: 11px;"><strong style="color: #4B5563;">Email:</strong> <span style="color: #1F2937;">${userDetails?.email || "N/A"}</span></p>
                        <p style="margin: 2px 0; font-size: 11px;"><strong style="color: #4B5563;">Role:</strong> <span style="color: #1F2937;">${userDetails?.role || "Student"}</span></p>
                    </div>
                    <div style="background: ${riskAssessment.color}15; padding: 8px; border-radius: 4px; border: 1px solid ${riskAssessment.color}30;">
                        <p style="margin: 2px 0; font-size: 11px;"><strong style="color: #4B5563;">Risk:</strong> 
                            <span style="color: ${riskAssessment.color}; font-weight: bold;">${riskAssessment.level}</span>
                        </p>
                        <p style="margin: 2px 0; font-size: 10px; color: #6B7280;">${riskAssessment.description}</p>
                    </div>
                </div>
            `;
            ...
            const lookingEvents = currentEvents.filter(e => e.event?.includes("Looking")).length;
            const cellPhoneEvents = currentEvents.filter(e => e.object === "cell phone").length;
            const avgConfidence = (() => {
                const eventsWithConfidence = currentEvents.filter(e => e.confidence !== undefined);
                if (eventsWithConfidence.length === 0) return 0;
                
                const totalConfidence = eventsWithConfidence.reduce((sum, event) => sum + (event.confidence || 0), 0);
            ...
            summaryDiv.innerHTML = `
                <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px;">
                    ...
                    <div style="padding: 8px; background: white; border-radius: 4px; box-shadow: 0 1px 1px rgba(0,0,0,0.1);">
                        ...
                        <p style="color: #4F46E5; font-size: 16px; font-weight: bold; margin: 2px 0;">${avgConfidence.toFixed(1)}%</p>
                    </div>
                </div>
            `;
            ...
            const createEventTable = (title: string, events: any[], type: 'looking' | 'object') => {
                ...
                tableDiv.innerHTML = `
                    ...
                    <table style="width: 100%; border-collapse: collapse; margin-bottom: 10px; font-size: 10px;">
                        <thead>
                            <tr style="background-color: #F3F4F6;">
                                ...
                                ${type === 'looking' ? '<th style="padding: 6px; text-align: left; border: 1px solid #E5E7EB;">Duration</th>' : 
                        ...
                        <tbody>
                            ${topEvents.map(event => `
                                <tr style="border-bottom: 1px solid #E5E7EB;">
                                    <td style="padding: 6px; border: 1px solid #E5E7EB;">${format(new Date(event.timestamp * 1000), "HH:mm:ss")}</td>
                                    <td style="padding: 6px; border: 1px solid #E5E7EB;">${type === 'looking' ? event.event : event.object}</td>
                                    <td style="padding: 6px; border: 1px solid #E5E7EB;">
                                        ${type === 'looking' ? 
                                            event.duration.toFixed(1) : 
                                            `<div style="background: #E5E7EB; height: 4px; border-radius: 2px;">
                                                <div style="background: #4F46E5; height: 100%; width: ${event.confidence}%; border-radius: 2px;"></div>
                                            </div>
                                            <span style="font-size: 9px; color: #6B7280;">${event.confidence.toFixed(0)}%</span>`
                                        }
                                    </td>
                    </tr>
                            `).join('')}
                    ...
                    ${events.length > 10 ? `<p style="font-size: 9px; color: #6B7280; text-align: right;">+${events.length - 10} more events</p>` : ''}
                `;
                return tableDiv;
            };
            ...
            const lookingEventsList = currentEvents
                ...
                .map(e => ({ 
                    ...e, 
                    timestamp: getEventTimestamp(e, attempts[currentAttempt]?.timestamp || 0)
                }));
            ...
            eventsDiv.appendChild(createEventTable("Looking Events", lookingEventsList, 'looking'));
            eventsDiv.appendChild(createEventTable("Cell Phone Detections", objectEventsList, 'object'));
            ...
            pdf.save(`EthicalReport_${userDetails?.username || "Student"}_${format(new Date(), "yyyyMMdd_HHmm")}.pdf`, {
                returnPromise: true
            }).then(() => {
    ...
    const renderLookingEventsTable = () => (
        <tbody className="bg-white divide-y divide-gray-200">
            {currentEvents.filter(e => e.event?.includes("Looking")).map((event, index) => {
                const eventTimestamp = getEventTimestamp(event, attempts[currentAttempt]?.timestamp || 0);
                return (
                    <tr 
    ...
    const renderPhoneEventsTable = () => (
        <tbody className="bg-white divide-y divide-gray-200">
            {currentEvents.filter(e => e.object === "cell phone").map((event, index) => {
                const eventTimestamp = getEventTimestamp(event, attempts[currentAttempt]?.timestamp || 0);
                return (
                    <tr 
                        key={`${eventTimestamp}-${index}`}
            ...
            <div className="max-w-6xl mx-auto p-4 sm:p-6 lg:p-8">
                    ...
                    <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                        <div>
                            <div className="flex items-center gap-4 mb-2">
                                <h1 className="text-2xl font-bold text-gray-800">Student Exam Cheating Report</h1>
                                <div className="flex items-center gap-2 bg-indigo-50 px-3 py-1 rounded-lg">
                                    <button
                                        ...
                                        className={`p-1 rounded ${currentAttempt === 0 ? 'text-gray-400' : 'text-indigo-600 hover:bg-indigo-100'}`}
                                    >
                                    ...
                                    <button
                                        ...
                                        className={`p-1 rounded ${currentAttempt === attempts.length - 1 ? 'text-gray-400' : 'text-indigo-600 hover:bg-indigo-100'}`}
                                    >
                            ...
                            <p className="text-gray-600">
                                Generated on {format(new Date(attempts[currentAttempt]?.timestamp * 1000 || Date.now()), "MMMM dd, yyyy 'at' HH:mm")}
                            </p>
                        </div>
                        <button
                            onClick={downloadPDF}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1 space-y-6">
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <h3 className="text-lg font-semibold text-gray-800">Attempt Summary</h3>
                            </div>
                            <div className="space-y-4">
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="bg-gray-50 rounded-lg p-3">
                                        ...
                                        <p className="font-semibold text-gray-800">
                                            {format(new Date(attempts[currentAttempt]?.timestamp * 1000 || Date.now()), "HH:mm:ss")}
                                        </p>
                                    </div>
                                    <div className="bg-gray-50 rounded-lg p-3">
                                ...
                                <div className="space-y-2">
                                    {Object.entries(
                                        currentEvents.reduce((acc: { [key: string]: number }, event) => {
                                            if (event.object) {
                                        ...
                                        <div key={object} className={`flex items-center justify-between p-2 rounded-lg ${getObjectColor(object)}`}>
                                            <div className="flex items-center gap-2">
                    ...
                    <div className="lg:col-span-2 space-y-6">
                            ...
                            {expandedSections.summary && (
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                    ...
                                    <div className="bg-gray-50 rounded-lg p-4 text-center">
                                        ...
                                        <p className="text-2xl font-bold text-indigo-600">
                                            {currentEvents.filter(e => e.event?.includes("Looking")).length}
                                        </p>
                                    </div>
                                    <div className="bg-gray-50 rounded-lg p-4 text-center">
                                        <p className="text-sm text-gray-600 mb-1">Phone Detections</p>
                                        <p className="text-2xl font-bold text-indigo-600">
                                            {currentEvents.filter(e => e.object === "cell phone").length}
                                        </p>
                                    </div>
                                    <div className="bg-gray-50 rounded-lg p-4 text-center">
                                        ...
                                        <p className="text-2xl font-bold text-indigo-600">
                                            {(() => {
                                                ...
                                                const totalConfidence = eventsWithConfidence.reduce((sum, event) => sum + (event.confidence || 0), 0);
                                                const average = totalConfidence / eventsWithConfidence.length;
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            <h3 className="text-lg font-semibold text-gray-800 mb-4">Events Distribution</h3>
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <div>
                                        <h3 className="text-lg font-semibold text-gray-800">Object Detections</h3>
                                        <p className="text-sm text-gray-500">Real-time detection confidence levels</p>
                                    </div>
                                </div>
                                {expandedSections.objectEvents ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </div>
                            {expandedSections.objectEvents && (
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {Object.entries(
                                        currentEvents.reduce((acc: { [key: string]: CheatingEvent[] }, event) => {
                                            if (event.object) {
                                                if (!acc[event.object]) {
                                                    acc[event.object] = [];
                                                }
                                        ...
                                        const totalEvents = events.length;
                                        const avgConfidence = events.reduce((sum, event) => sum + (event.confidence || 0), 0) / totalEvents;
                                        const highConfidenceEvents = events.filter(e => (e.confidence || 0) > 70).length;
                                        const riskLevel = highConfidenceEvents > 0 ? 
                                            (highConfidenceEvents / totalEvents > 0.5 ? 'high' : 'medium') : 'low';

                                        return (
                                            <div key={object} className="bg-gray-50 rounded-lg overflow-hidden border border-gray-100">
                                                <div className="p-3 bg-white border-b border-gray-100">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center gap-2">
                                                            {getObjectIcon(object)}
                                                            <div>
                                                                <h4 className="font-semibold text-gray-800 capitalize">{object}</h4>
                                                                    ...
                                                                    <div 
                                                                        className={`h-full rounded-full transition-all duration-300 ${
                                                                            event.confidence && event.confidence > 70 
                                                                                ? 'bg-red-500' 
                                                                                : event.confidence && event.confidence > 50 
                                                                                ? 'bg-yellow-500' 
                                                                                : 'bg-green-500'
                                                                        }`}
                                                                        style={{ width: `${event.confidence}%` }}
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    <Eye className="w-5 h-5 text-indigo-600" />
                                    <h3 className="text-lg font-semibold text-gray-800">Looking Events</h3>
                                </div>
                                {expandedSections.lookingEvents ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </div>
                        ...
                        <div className="bg-white rounded-xl shadow-lg p-6">
                            ...
                            >
                                <div className="flex items-center gap-2">
                                    ...
                                    <h3 className="text-lg font-semibold text-gray-800">Phone Detections</h3>
                                </div>
                                {expandedSections.phoneEvents ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
                            </div>
