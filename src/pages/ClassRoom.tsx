        ...
        videoRef.current.onloadedmetadata = () => {
          videoRef.current
            ...
            .catch((err) => {
              ...
              setCameraError(
                "Could not play video stream. Please check permissions."
              );
            });
        };
      }
  ...
  const sendFrame = async (frame: HTMLVideoElement) => {
    ...
    try {
      ...
      if (drowsinessResponse.status === 200) {
        ...
        console.error(
          "Error processing drowsiness:",
          drowsinessResponse.statusText
        );
      }
      ...
      if (emotionResponse.status === 200) {
        ...
        sendToFirebase(emotionResult.emotion);
      ...
      if (error.response && error.response.status === 401) {
        ...
        setCameraError(
          "Connection to detection server lost. Please reconnect."
        );
      }
    } finally {
      setIsSendingFrame(false);
